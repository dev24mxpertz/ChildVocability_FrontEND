...
import { fetchAllHistory, fetchCompletedHistoryfiction, fetchInProgressHistoryfiction, fetchNewHistoryfiction } from '../../store/Actions/storyActions'
...
const History = () => {
    ...
    const fantasys =  useSelector((state) => state.Story.AllHistory)
...
const getStatusButtonColor = (status) => {
  ...
  const Completed_Story_Handler = () => {
    dispatch(fetchCompletedHistoryfiction())
  }

  const InProgress_Story_Handler = () => {
    dispatch(fetchInProgressHistoryfiction())
  }

  const NotStarted_Story_Handler = () => {
    dispatch(fetchNewHistoryfiction())
  }
  ...
  return (
    ...
    <div className='headerdiv'>
          ...
          <div className='storiesnadbutton'>
                    <div>
                    <h4 style={{ fontWeight: 800 }}>History Stories</h4>
                    </div>
              ...
              {fantasys?.length > 0 ? (
                  fantasys?.map((data) => (
                    ...
                    <div>
                        ...
                        <h6 onClick={()=> Historytab(data?._id)} className='tab-h6-style'>{data?.Title}</h6>
                    </div>
